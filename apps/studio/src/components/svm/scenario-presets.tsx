  ...
  return (
    <section aria-labelledby="scenario-presets-heading" className="mx-auto mb-8 max-w-7xl px-6 lg:px-8">
      ...
      <div className="mx-auto grid max-w-4xl gap-3 md:grid-cols-2">
        ...
        <button
          type="button"
          onClick={onPancakeswapStateSelect}
          className="group flex items-center gap-4 rounded-2xl border border-zinc-700/50 bg-zinc-900/60 p-4 text-left shadow-lg shadow-black/10 transition-colors hover:border-yellow-500/40 hover:bg-zinc-900"
        >
          <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-zinc-800 ring-1 ring-zinc-700/70">
            <Image src={getProtocolIcon('pancakeswap')} alt="" width={28} height={28} />
          </span>
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-semibold text-zinc-100">PancakeSwap state</span>
            <span className="mt-1 block text-sm leading-5 text-zinc-500">
              Shock a CLMM pool&apos;s price or change a CLMM fee tier.
            </span>
          </span>
          <ArrowRightIcon className="size-4 shrink-0 text-zinc-600 transition-colors group-hover:text-yellow-400" />
        </button>
      </div>
    </section>
  );
