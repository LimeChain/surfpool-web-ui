...
export default function ScenariosBento({
  ...
  const [pancakeswapStateDialogOpen, setPancakeswapStateDialogOpen] = useState(false);
  ...
  const handleOpenPancakeswapStateDialog = () => {
    setPancakeswapStateDialogOpen(true);
  };

  const handleClosePancakeswapStateDialog = () => {
    setPancakeswapStateDialogOpen(false);
  };

  const handlePancakeswapStateCreated = (scenarioId: string) => {
    setPancakeswapStateDialogOpen(false);
    onRefresh?.();
    router.push(`/scenarios?id=${scenarioId}&tab=editor`);
  };
  ...
  return (
    <div className="relative h-full">
      ...
      {!isDetailPaneOpen && (
        <>
          ...
          <ScenarioPresets
            ...
            onPancakeswapStateSelect={handleOpenPancakeswapStateDialog}
          />
        </>
      )}
      ...
      <PancakeswapStateDialog
        open={pancakeswapStateDialogOpen}
        studioUrl={studioUrl}
        onClose={handleClosePancakeswapStateDialog}
        onCreated={handlePancakeswapStateCreated}
      />
    </div>
  );
}
