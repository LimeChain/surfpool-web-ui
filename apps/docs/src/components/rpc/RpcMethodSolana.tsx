...
export function RpcMethodSolana({ endpoint }: RpcMethodSolanaProps) {
  ...
  const topLevelParams = endpoint.params.filter(p => !p.parentName && !p.isObjectHeader);
  const objectHeaders = endpoint.params.filter(p => p.isObjectHeader && !p.parentName);
  ...
  const addObjectWithChildren = (header: typeof endpoint.params[0], indentLevel: number) => {
    ...
    <div className="mb-8 pb-8 border-b border-fd-border last:border-0">
      ...
      <h3 id={endpoint.method_name} className="flex items-center gap-2 text-base font-semibold text-fd-foreground font-mono mb-1 scroll-mt-20">
        {endpoint.status === "implemented" ? (
          ...
          <span className="w-2 h-2 rounded-full bg-yellow-500 shrink-0" title="Coming Soon" />
        )}
              ...
              {allParams.map((param, i) => (
                <tr key={i} className="border-b border-fd-border/30 last:border-0">
                  <td className="py-2 pr-4">
                    <div className="flex items-center gap-1.5" style={{ paddingLeft: `${param.indent * 16}px` }}>
                      {param.indent > 0 && <span className="text-fd-border text-xs">└</span>}
      ...
      {resultFields.length > 0 && (
        <div className="mb-4">
          <div className="text-[11px] font-semibold text-fd-muted-foreground uppercase tracking-wide mb-2">Result</div>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] text-fd-muted-foreground border-b border-fd-border/50">
            ...
            <tbody className="align-top">
              {resultFields.map((field, i) => (
                <tr key={i} className="border-b border-fd-border/30 last:border-0">
                  ...
                  <td className="py-2 text-fd-muted-foreground text-[13px]">{field.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div>
        ...
        <div className="rounded-lg border border-fd-border overflow-hidden">
          ...
          <div className="[&_figure]:m-0 [&_figure]:rounded-none [&_figure]:border-0 [&_pre]:rounded-none [&_pre]:border-0">
            <DynamicCodeBlock
              lang="json"
              code={activeTab === "request" ? endpoint.example_request : endpoint.example_response}
            />
          </div>
        </div>
      </div>
