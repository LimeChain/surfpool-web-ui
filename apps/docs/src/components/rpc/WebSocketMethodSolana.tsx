...
export function WebSocketMethodSolana({ endpoint }: WebSocketMethodSolanaProps) {
  ...
  const getCode = (tab: TabType): string => {
    switch (tab) {
      case "subscribe": return endpoint.subscribe_example;
      case "response": return endpoint.subscribe_response_example;
      case "notification": return endpoint.notification_example;
      case "unsubscribe": return `${endpoint.unsubscribe_example}\n\n// Response\n${endpoint.unsubscribe_response_example}`;
      default: return "";
    }
  };
  ...
  const extractFields = (obj: any, prefix: string = '', indent: number = 0) => {
    ...
    if (Array.isArray(obj)) {
      if (obj.length > 0 && typeof obj[0] === 'object' && obj[0] !== null) {
        ...
        resultFields.push({ name: prefix || 'result', type: `array<${itemType}>`, indent });
      }
      return;
    }
    ...
    if (typeof obj === 'object') {
      ...
      Object.entries(obj).forEach(([key, value]) => {
        if (value === null) {
          resultFields.push({ name: key, type: 'null', indent: prefix ? indent + 1 : indent });
        } else if (Array.isArray(value)) {
          if (value.length > 0 && typeof value[0] === 'object' && value[0] !== null) {
            resultFields.push({ name: key, type: 'array', indent: prefix ? indent + 1 : indent });
            extractFields(value[0], '[]', prefix ? indent + 2 : indent + 1);
          } else {
            const itemType = value.length > 0 ? typeof value[0] : 'any';
            resultFields.push({ name: key, type: `array<${itemType}>`, indent: prefix ? indent + 1 : indent });
          }
        } else if (typeof value === 'object') {
          extractFields(value, key, prefix ? indent + 1 : indent);
        } else {
          resultFields.push({ name: key, type: typeof value, indent: prefix ? indent + 1 : indent });
        }
      });
      return;
    }
  ...
  return (
    <div className="mb-8 pb-8 border-b border-fd-border last:border-0">
      {/* Header */}
      <h3 id={endpoint.method_name} className="flex items-center gap-2 text-base font-semibold text-fd-foreground font-mono mb-1 scroll-mt-20">
        <span className="w-2 h-2 rounded-full bg-purple-500 shrink-0" title="WebSocket" />
        {endpoint.method_name}
      </h3>
      <p className="text-sm text-fd-muted-foreground mb-4">{endpoint.description}</p>

      {/* Parameters Table */}
      {endpoint.params.length > 0 && (
        <div className="mb-4">
          <div className="text-[11px] font-semibold text-fd-muted-foreground uppercase tracking-wide mb-2">Parameters</div>
          <table className="w-full text-sm">
            ...
            <tbody className="align-top">
              {endpoint.params.map((param, i) => (
                <tr key={i} className="border-b border-fd-border/30 last:border-0">
                  <td className="py-2 pr-4">
                    <div className="flex items-center gap-1.5">
                      <code className="text-fd-foreground font-mono text-[13px]">{param.name}</code>
                  ...
                  <td className="py-2 text-fd-muted-foreground text-[13px]">{param.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {resultFields.length > 0 && (
        <div className="mb-4">
          <div className="text-[11px] font-semibold text-fd-muted-foreground uppercase tracking-wide mb-2">Notification Result</div>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] text-fd-muted-foreground border-b border-fd-border/50">
            ...
            <tbody className="align-top">
              {resultFields.map((field, i) => (
                <tr key={i} className="border-b border-fd-border/30 last:border-0">
                  <td className="py-2 pr-4">
                    <div className="flex items-center gap-1.5" style={{ paddingLeft: `${field.indent * 16}px` }}>
                      {field.indent > 0 && <span className="text-fd-border text-xs">└</span>}
                      <code className="text-fd-foreground font-mono text-[13px]">{field.name}</code>
                    </div>
                  </td>
                  <td className="py-2 pr-4 whitespace-nowrap">
      ...
      <div>
        <div className="text-[11px] font-semibold text-fd-muted-foreground uppercase tracking-wide mb-2">Example</div>
        <div className="rounded-lg border border-fd-border overflow-hidden">
          ...
          {/* Code Content */}
          <div className="[&_figure]:m-0 [&_figure]:rounded-none [&_figure]:border-0 [&_pre]:rounded-none [&_pre]:border-0">
            <DynamicCodeBlock
              lang="json"
