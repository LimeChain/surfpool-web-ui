...
interface RpcPageContentProps {
  category: "transactions" | "accounts" | "cheatcodes" | "node" | "network" | "admin";
}
...
const categoryDescriptions: Record<string, string> = {
  transactions: "These methods handle the creation, simulation, and inspection of transactions on the blockchain. They are essential for executing state changes, estimating fees, checking transaction outcomes, and monitoring transaction lifecycles.",
  accounts: "Query account states, balances, token holdings, and program-specific storage data.",
  cheatcodes: "Powerful testing utilities unique to Surfpool that allow you to directly modify account and token states. These methods are only available on Surfnet.",
  node: "Monitor node status, health checks, cluster information, and validator details.",
  network: "Access network-wide information like epoch data, inflation rates, and performance metrics.",
  admin: "Administrative controls for managing the Surfnet instance, plugins, and system configuration.",
};
  ...
  return (
    <div>
      ...
      {endpoints.map((endpoint, index) => {
        const id = endpoint.method_name.toLowerCase().replace(/[^a-z0-9]/g, '-');
        return (
          <div key={index} id={id}>
