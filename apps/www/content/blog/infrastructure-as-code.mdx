...
We think we have a strong foundation to address this problem: **Infrastructure as Code for Web3**.
...
Each of these is a transaction that needs to be signed, broadcast, and confirmed. Each one can fail. Each one needs to be auditable.
...
action "setup" "svm::setup_surfnet" {
    ...
    // Reset an account to empty state
    reset_account {
        public_key = variable.test_account
    }
}
```
...
```bash
surfpool run deployment --supervised
```
...
Start Surfpool with `--watch` and programs auto-redeploy on recompile:

```bash
surfpool start --watch
```

Change your Rust code, save, and Surfpool handles the rest.
...
- Web wallet signing with `--supervised`
